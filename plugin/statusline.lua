...
M.get_file_encoding = function()
  return string.format('%s ', vim.o.fileencoding)
end
...
M.select_indentation = function()
  vim.ui.select({'tabs', 'spaces'}, {
    ...
    local width = tonumber(fn.input('Enter indentation width: '))
...
M.selection = function(mode)
  if mode[1] ~= 'V' and mode[1] ~= 'v' and mode[1] ~= '\22' then return "" end
...
M.set_active = function(self)
  ...
  local gitdiff = vim.b.gitsigns_status
  local githead = vim.b.gitsigns_head
  ...
  return table.concat({
    ...
    ' %1p%%/%L ',                                      -- Place in file as a percentage
  })
end
